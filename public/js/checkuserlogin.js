$(document).ready(function () {
    ...
    $('.custom-login').click(function (e) { 
        e.preventDefault();
        if(cipher == null) {
            window.location.replace("http://localhost:88/ProjectWeb/Login");
        } else {
            //Nếu đăng nhập rồi, click vào sẽ nhảy qua trang thông tin khách hàng
              window.location.replace("http://localhost:88/ProjectWeb/Profile/User/"+obj.id);
        }
    ...
    $('.custom-logout').click(function (e) { 
        ...
        window.location.replace("http://localhost:88/ProjectWeb");
    });
